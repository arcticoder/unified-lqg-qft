...
class WarpBubbleAnalysis:
    """
    Wrapper class for warp bubble analysis functionality.
    Provides a unified interface to all warp bubble analysis functions.
    """
    
    def __init__(self, mu_default=0.5, tau_default=1.0, R_default=3.0):
        self.mu_default = mu_default
        self.tau_default = tau_default
        self.R_default = R_default
    
    def run_analysis(self, **kwargs):
        """Run complete warp bubble analysis."""
        return run_warp_analysis(**kwargs)
    
    def scan_parameter_space(self, mu_vals=None, tau_vals=None, R_vals=None):
        """Scan parameter space for optimal configurations."""
        if mu_vals is None:
            mu_vals = [0.1, 0.5, 1.0]
        if tau_vals is None:
            tau_vals = [0.5, 1.0, 2.0]
        ...
        return scan_3d_shell(mu_vals, tau_vals, R_vals, 
                           sigma=0.3, A_factor=1.0, omega=1.0)
    
    def compute_negative_energy(self, mu=None, tau=None, R=None):
        """Compute negative energy requirements."""
        mu = mu or self.mu_default
        tau = tau or self.tau_default
        R = R or self.R_default
        
        return required_negative_energy(mu, tau, R)
    
    def stability_analysis(self, phi_0, pi_0, mu=None):
